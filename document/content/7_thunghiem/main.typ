...
== Xây dựng bộ dữ liệu hỏi đáp luật
...
Sau khi có được bộ dữ liệu hỏi đáp luật, tôi tiếp tục sử dụng Label Studio@LabelStudio để gán nhãn cho câu trả lời.
...
#table(
    ...
    // bold col
    ..col_kq.map((x) => text(weight: "bold",x)),
    ..ketqua.slice(1).flatten()
)
